...
/**
TokeNestUpdate : TokeNest에서 사용하지 않는 메서드들 주석처리.
 */
contract DexRouter is IDexRouter {
    ...
    constructor(address _factory) {
        ...
        // if(_tokenAddress == address(0)) revert InvalidAddressParameters("DexRouter: WKLAY_ZERO_ADDRESS");
        factory = _factory;
        // WKLAY = _tokenAddress;
    }
    ...
    // **** ADD LIQUIDITY ****
    /**
     TokeNestUpdate : DexLibrary의 pairFor을 사용하지 않으므로 pairFor(factory, tokenA, tokenB) -> IDexFactory(factory).getPair(tokenA, tokenB)로 변경.
     추가로 DexLibrary의 getReserves 메서드도 변경됨.
     */
    function _addLiquidity(
        ...
        if (IDexFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            revert InvalidAddressParameters("Error : Not Exist Pair. tokenA<>tokenB");
            /**
            TokeNestUpdate : 기존 pair가 없을 시 생성하는 것이 아닌 오류 띄우기.
             */
            // IDexFactory(factory).createPair(tokenA, tokenB, "TOKENEST", "TKNLP");
        }
        (uint256 reserveA, uint256 reserveB) = DexLibrary.getReserves(
            IDexFactory(factory).getPair(tokenA, tokenB),
            tokenA,
            tokenB
        );
     ...
     */
    /**
    TokeNestUpdate : DexLibrary.pairFor(factory, tokenA, tokenB); -> IDexFactory(factory).getPair(tokenA, tokenB); 변경
     */
    ...
    */
    ...
    /**
    TokeNestUpdate : DexLibrary.pairFor -> IDexFactory(factory).getPair 변경.
    메서드 설명 : path[0]을 amountIn만큼 넣어서 path[1]로 스왑하겠다라는 뜻.
    ex) 10000000, 0, ["token1", "token2"], to, deadline  <= token1 10개를 소비해서 스왑되는 token2를 to주소로 보내겠다.
     */
    ...
    {
        amounts = DexLibrary.getAmountsOut(IDexFactory(factory).getPair(path[0], path[1]), amountIn,  path);
    ...
    /**
    TokeNestUpdate : DexLibrary.pairFor -> IDexFactory(factory).getPair 변경.
    메서드 설명 : path[1]을 amountOut만큼 얻기 위해 path[0]을 구해서 스왑하겠다는 뜻.
    ex) 10000000, constants.MaxUint256, ["token1", "token2"], to, deadline  <= token1 x개와 token2를 10개를 스왑해 to로 보내겠다.
     */
    function swapTokensForExactTokens(
        ...
        returns (uint256[] memory amounts)
    {
        amounts = DexLibrary.getAmountsIn(IDexFactory(factory).getPair(path[0], path[1]), amountOut, path);
    ...
    //         path[0],
    //         msg.sender,
    //         DexLibrary.pairFor(factory, tokenA, tokenB),
    //         amounts[0]
    //     );
    //     _swap(amounts, path, to);
    // }
