...
**TokeNest** is a blockchain DEX platform that tracks price fluctuations in real-time and adjusts prices accordingly.

In the traditional market, it has often been challenging to adjust prices when price fluctuations occur, leading to dissatisfaction among consumers and businesses. **TokeNest** was created to address this issue by utilizing blockchain technology and smart contracts.

In the TokeNest_SmartContract repository, you can issue and manage tokens that correspond to real-world assets within the Klaytn ecosystem. In the TokeNest_NextJs repository, a kiosk system has been implemented that adjusts prices of real-time products based on tokens issued on the blockchain.

By utilizing both repositories, you can implement a system where any product can be tokenized and prices can adjust in real-time according to market fluctuations.
...
**TokeNest** DEX Contracts are smart contracts that reflect real-time market prices of products on the Klaytn blockchain and calculate amounts accordingly.

This repository is a result of additional work based on the **Klaytn-Dex-Contract** repository. It has been cloned and modified to align with TokeNest's specifications.

**Klaytn-Dex-Contract Repository**: [Klaytn-Dex-Contract](https://github.com/klaytn/klaytn-dex-contracts)

## Main Mechanism

- **Real-time Price Adjustment**: It provides fair trading by tracking price fluctuations in real-time and adjusting prices.
- **Tokenized Real-World Assets**: It enables the trading of tokenized real-world assets that respond to price fluctuations in real-time.
- **Reliability and Transparency**: It records all transaction details transparently in a decentralized distributed ledger on the blockchain.
- **Streamlining Distribution Processes**: It automates transactions and streamlines the distribution process using smart contracts.

## Scenario

TokeNest DEX Contracts operate through the following scenario:

1. **Tokenization**: Sellers create tokens corresponding to real-world assets and list them on the blockchain.
2. **Swap**: Buyers swap tokens using stablecoins on the DEX.
3. **NFT Issuance**: Buyers receive NFTs to exchange tokens for real-world assets (feature in progress).
4. **NFT Burn**: Upon completion of the transaction, NFTs are burned to receive real-world assets (feature in progress).
5. **Production of Finished Products**: Buyers process real-world assets into finished products and sell them.
6. **Kiosk System**: Consumers track real-time prices and purchase products at reasonable prices using a kiosk system. ([TokeNest-NextJs](https://github.com/TokeNest/TokeNest_NextJs))
